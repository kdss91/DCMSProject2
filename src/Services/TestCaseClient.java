...
public class TestCaseClient extends Thread{
	...
	public void run() {
		CORBAImplementation beginserver = null;
		...
		try {
			...
			String nameofserver = sName + "Server"; 
			beginserver = CORBAImplementationHelper.narrow(newreferencethere.resolve_str(nameofserver));
		...
		try {
			...
			if (isSignedIn) {
				...
				//Transfer Teacher record from MTL to DDO
				if(this.managerID.startsWith("MTL")){
					beginserver.transferRecord(managerID, "TR99996", "DDO"); 
				}
